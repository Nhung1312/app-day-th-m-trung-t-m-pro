...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (hasAccess) {
            await loadDataFromProCloud(user.uid);
...
async function loadDataFromProCloud(uid) {
    showLoading(true, "Đang đồng bộ dữ liệu từ Cloud...");
    try {
        const newDb = JSON.parse(JSON.stringify(defaultData));
        const legacyDoc = await getDoc(doc(firestoreDb, "DuLieuDayThem", uid));
        ...
        if (!metaDoc.exists() && legacyDoc.exists()) {
            ...
            const collections = ['classes', 'students', 'holidays', 'sessions', 'attendance', 'tuitions'];
            for (let col of collections) {
                const snap = await getDocs(collection(firestoreDb, `UsersPro/${uid}/${col}`));
                snap.forEach(d => newDb[col].push(d.data()));
            }
            let metaData = metaDoc.data();
            newDb.settings = metaData.settings || newDb.settings;
            newDb.processedTx = metaData.processedTx || [];
            db = Object.assign({}, defaultData, newDb);
        }
...
async function saveData() {
    ...
    if (currentUser) {
        ...
        syncTimeout = setTimeout(() => {
            smartSyncToFirebase(currentUser.uid);
        }, 2000); 
    }
}

async function smartSyncToFirebase(uid) {
    if (!lastSyncedDb) return;
    let operations = [];
    const collections = ['classes', 'students', 'holidays', 'sessions', 'attendance', 'tuitions'];

    for (let col of collections) {
        const currentArray = db[col];
        const oldArray = lastSyncedDb[col] || [];

        for (let item of currentArray) {
            const oldItem = oldArray.find(x => String(x.id) === String(item.id) || (col === 'attendance' && x.sessionId === item.sessionId && x.studentId === item.studentId));
            if (!oldItem || JSON.stringify(item) !== JSON.stringify(oldItem)) {
                let docId = item.id ? String(item.id) : `${item.sessionId}_${item.studentId}`;
                operations.push({ type: 'set', ref: doc(firestoreDb, `UsersPro/${uid}/${col}`, docId), data: item });
            }
        }
        for (let oldItem of oldArray) {
            const stillExists = currentArray.find(x => String(x.id) === String(oldItem.id) || (col === 'attendance' && x.sessionId === oldItem.sessionId && x.studentId === oldItem.studentId));
            if (!stillExists) {
                let docId = oldItem.id ? String(oldItem.id) : `${oldItem.sessionId}_${oldItem.studentId}`;
                operations.push({ type: 'delete', ref: doc(firestoreDb, `UsersPro/${uid}/${col}`, docId) });
            }
        }
    }

    if (JSON.stringify(db.settings) !== JSON.stringify(lastSyncedDb.settings) || JSON.stringify(db.processedTx) !== JSON.stringify(lastSyncedDb.processedTx)) {
        operations.push({ type: 'set', ref: doc(firestoreDb, `UsersPro/${uid}/config`, 'meta'), data: { settings: db.settings, processedTx: db.processedTx } });
    }
    ...
    if (operations.length > 0) {
        try {
            ...
            lastSyncedDb = JSON.parse(JSON.stringify(db));
            console.log(`Đã đồng bộ lên Cloud: ${operations.length} thay đổi`);
...
function updateDashboard() {
    ...
    db.tuitions = db.tuitions.filter(t => db.students.some(st => st.id == t.studentId)); 
    ...
    db.classes.forEach(c => {
        ...
        dueCount += tData.filter(d => !d.isPaid).length;
    });
    const dashDue = document.getElementById('dash-tuition-due');
    if(dashDue) dashDue.innerText = dueCount;
...
function renderTuition() {
    ...
    let cid = select ? select.value : null; 
    
    const list = document.getElementById('tuition-due-list'); 
    if(list) list.innerHTML = '';
    
    if(!cid) {
        if(list) list.innerHTML = '<div class="text-center text-muted" style="padding:30px;">Vui lòng tạo nhóm lớp trước!</div>';
        return;
    }
    ...
    data.forEach(d => {
        ...
        let statusBtn = d.isPaid 
            ? `<button class="t-badge paid" onclick="toggleTuition(${d.stu.id}, ${d.cls.id}, ${d.cycleNum}, ${d.finalAmount})">✔ Đã nộp</button>`
            : `<button class="t-badge unpaid" onclick="toggleTuition(${d.stu.id}, ${d.cls.id}, ${d.cycleNum}, ${d.finalAmount})">❌ Chưa nộp</button>`;

        if(list) {
            list.innerHTML += `
                <div class="tuition-card" style="margin-bottom: 10px;">
                    <div style="flex:1;">
                    ...
                    <div style="text-align:right;">
                        ...
                        <div style="display:flex; gap:5px; margin-top:8px; justify-content:flex-end;">
                            <button class="btn-sm text-blue btn-outline-action" onclick="window.open('${d.qrLink}')" title="Mã QR"><i class="fas fa-qrcode"></i></button>
                            <button class="btn-sm text-green btn-outline-action" onclick="remindZalo('${d.stu.name}', '${d.ckContent}', ${d.finalAmount}, '${d.stu.phone || ''}')" title="Nhắc Zalo"><i class="fas fa-comment-dots"></i></button>
                        </div>
                    </div>
                </div>`;
        }
    ...
    const elDuThu = document.getElementById('t-du-thu'); if(elDuThu) elDuThu.innerText = tongDuThu.toLocaleString() + 'đ';
    ...
    if (histList) {
        ...
        if(sortedTuitions.length === 0) {
            ...
            sortedTuitions.forEach(t => {
                ...
                let cls = stu ? db.classes.find(c => c.id == stu.classId) : null;
                histList.innerHTML += `
                    <div class="list-item">
                        <div class="list-item-info">
                            ...
                            <small class="text-muted">${parseDateVi(t.date)} • Đóng cho Kỳ ${t.cycleNumber || 1}</small>
                        </div>
                        <button class="btn-outline-action text-red" title="Hủy giao dịch" onclick="deleteTuition(${t.id})"><i class="fas fa-trash"></i></button>
                    </div>`;
            });
        }
    }
...
function toggleTuition(stuId, classId, cycleNum, amount) {
    let exist = db.tuitions.find(t => t.studentId == stuId && t.cycleNumber === cycleNum);
    if(exist) {
        db.tuitions = db.tuitions.filter(t => t.id !== exist.id);
        showToast("Đã chuyển về Chưa nộp!");
    } else {
        db.tuitions.push({ id: Date.now(), studentId: stuId, classId: classId, cycleNumber: cycleNum, amount: amount, date: getTodayStr() });
        showToast("✅ Đã ghi nhận thu tiền thành công!");
    }
    saveData(); 
    renderTuition();
}
...
function remindZalo(name, ck, amount, phone) {
    let msg = `[THÔNG BÁO] Trung tâm gửi thông tin học phí kỳ tới của học sinh ${name}.\n💰 Số tiền cần đóng: ${amount.toLocaleString()}đ.\n📌 Cú pháp chuyển khoản: ${ck}\nKính mong Phụ huynh kiểm tra và hoàn thành sớm. Xin cảm ơn!`;
    navigator.clipboard.writeText(msg).then(() => {
        if (phone && phone.trim() !== '') {
            window.open(`https://zalo.me/${phone}`, '_blank');
            showToast("✅ Đã copy và mở Zalo để gửi!");
        } else {
            showToast("✅ Đã copy! (Học sinh chưa có SĐT để mở Zalo)");
        }
    });
}
...
function copyClassNotice() {
    ...
    if(!cls) return showToast("Chưa chọn nhóm lớp!", "error");
    ...
    msg += `- Ngân hàng: ${bankId}\n`;
    msg += `- Số tài khoản: ${bankAcc}\n`;
    msg += `📌 Cú pháp: HP [Mã HS] (Hoặc quét Mã QR ở ảnh bên dưới)\n\n`;
    msg += `(Trân trọng cảm ơn Quý phụ huynh!)`;
    
    navigator.clipboard.writeText(msg).then(() => {
        showToast("✅ Đã copy thông báo. Dán (Paste) vào Nhóm Zalo nhé!");
    });
}
...
function openAddStudentForClass(cid) { 
    ...
    document.getElementById('stu-custom-fee').value = ''; 
    document.getElementById('stu-start').value = cls ? cls.startDate : getTodayStr(); 
    document.getElementById('stu-class').value = cid; 
...
function saveStudent() { 
    ...
    let obj = { 
        ...
        startDate: document.getElementById('stu-start').value || defaultStart,
        status: 'active'
    };
    ...
    if(id) {
        ...
        if(idx > -1) {
            obj.status = db.students[idx].status || 'active'; 
            db.students[idx] = obj;
        }
    } else {
...
function editStudent(id) {
    ...
    document.getElementById('stu-start').value = s.startDate || (cls ? cls.startDate : getTodayStr());
    openModal('modal-add-student');
}
...
function confirmImport() { 
    ...
    renderStudents(); 
    showToast(`🎉 Đã nhập ${count} học sinh!`); 
}
